...
import { GuaranteedProps, joinName, useForm } from 'uniforms/es5';
...
>(
  ...
  // @ts-ignore
  const joinedName = options && options.absoluteName ? '' : context.name;
  const name = joinName(joinedName, fieldName);
  ...
  if (options && options.initialValue !== false) {
